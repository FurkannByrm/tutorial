#include "cpp_service/my_server.hpp"


Server::Server():Node("cpp_server_node"){
    RCLCPP_INFO(this->get_logger(),"server has been started");
    server_ = this->create_service<example_interfaces::srv::AddTwoInts>("server_test", std::bind(&Server::serverCallback, this, std::placeholders::_1, std::placeholders::_2 ));
}
